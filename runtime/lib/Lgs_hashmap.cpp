...
extern "C" void Lgs_Map_add(Lgs_Map* map, const char* key, const void* value) {
...
extern "C" void* Lgs_Map_get(Lgs_Map* map, const char* key) {
...
extern "C" void Lgs_Map_delete(Lgs_Map* map, const char* key) {
    if (!map || !key) std::exit(1);
    map->data->erase(key);
}
