...
LgsMainFile* LgsParser::parseMainFile() {
    ...
    while (!isEOF()) {
        if (const auto obj = parseObject()) {
            ...
            mainFile->ioPairs.emplace_back(ioPair);
        } else {
            addParsingError();
            break;
        }
...
LgsAppFile* LgsParser::parseAppFile() {
    if (!scanTokens()) return nullptr;
...
LgsForLoop* LgsParser::parseForLoop() {
    ...
    forLoop->stmtsBlock = parseStmtsBlock();
    mustParse(forLoop->stmtsBlock);
    if (forLoop->stmtsBlock && forLoop->stmtsBlock->isMacro) addParsingError();
    setLocation(forLoop->location, &forKeyword, &currentToken);
    return forLoop;
}
...
LgsExpr* LgsParser::parseBinExpr(const int minPrecedence, const bool withLambda) {
    ...
    while (true) {
        ...
        if (matchAndConsume(T_ARROW)) {
            ...
            mustParse(toType);
            const auto cast = new LgsCast(left, toType);
            cast->location = left->location;
            return cast;
        }
        const LgsBinOp* op = nullptr;
...
void LgsParser::parseImports() {
    ...
    if (currentToken.lexeme == LGS_C) {
        ...
        while (true) {
            ...
            LgsImport importStmt(LGS_C_IMPORT, currentToken.lexeme);
        ...
        while (true) {
            const auto var = currentToken;
            if (!mustMatch(T_IDENTIFIER)) break;
            LgsImport importStmt(LGS_IMPORT, currentToken.lexeme);
            setLocation(importStmt.location, &importToken, &currentToken);
            importPaths.emplace_back(importStmt);
