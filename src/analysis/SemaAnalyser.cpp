...
void SemaAnalyser::visitForeachLoop(LgsForeachLoop* foreachLoop) {
    ...
    if (!iterable) {
        ...
        if (iterExpr->type) errHandler.handleError(E10002, &iterExpr->location, {iterExpr->pName()});
        return;
    }
...
bool SemaAnalyser::setLoopVars(LgsForeachLoop* foreachLoop, LgsUnaryExpr* iterExpr, const LgsIterable* iterable) {
    ...
    if (withIndex) {
        ...
        errHandler.handleError(E10041, &iterExpr->location, {iterExpr->pName(), to_string(iterable->unpackLength), to_string(iterable->unpackLength + 1), to_string(varDecSize)});
        return true;
    }
...
void SemaAnalyser::resolveFuncCall(LgsFuncCall* funcCall) {
    ...
    if (symbol->symbolType == FUNC) {
        ...
        if (funcCall->equals(func->funcType)) {
            ...
            errHandler.handleError(E10015, &funcCall->location, {funcCall->name, funcCall->pName(), func->pName()});
            return;
        }
