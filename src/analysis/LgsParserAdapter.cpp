...
void LgsParserAdapter::setAppConfigs() {
    ...
    setLocation(lgsAppFile.location, ctx->start, ctx->stop);
...
LgsObject* LgsParserAdapter::getObject(LogosParser::ObjectBodyContext* ctx, antlr4::tree::TerminalNode* objName, const bool isSingleton) {
    ...
    setLocation(obj->location, objName->getSymbol(), nullptr);
...
LgsInterface* LgsParserAdapter::getInterface(LogosParser::InterfaceBodyContext* ctx, antlr4::tree::TerminalNode* interfaceName) {
    ...
    for (const auto& interfaceFunc : ctx->interfaceFunc()) {
        ...
        currentFunc = func;
        setLocation(func->location, funcName->getSymbol(), nullptr);
...
LgsFunc* LgsParserAdapter::getFunc(LogosParser::FuncContext* ctx) {
    ...
    currentFunc = func;
    setLocation(func->location, funcNameToken->getSymbol(), nullptr);
...
LgsFunc* LgsParserAdapter::getLambda(LogosParser::LambdaContext* ctx) {
    ...
    if (const auto singleParam = ctx->IDENTIFIER()) {
        ...
        for (const auto lambdaParam : params->lambdaParam()) {
            ...
            setLocation(lgsParam.location, param->getSymbol(), nullptr);
            func->funcType->params.push_back(lgsParam);
        }
    }
    func->stmtsBlock = getStmtBlock(ctx->statementsBlock());
    setLocation(func->location, ctx->start, ctx->stop);
    currentFunc = nullptr;
    return func;
}
...
LgsIOPair* LgsParserAdapter::getIOPair(const LogosParser::IoPairContext* ctx) const {
    const auto lgsIOPair = new LgsIOPair(ctx->opening->getText(), ctx->closing->getText());
    setLocation(lgsIOPair->location, ctx->start, ctx->stop);
    return lgsIOPair;
}
...
LgsStmt* LgsParserAdapter::getDeferStmt(LogosParser::DeferStmtContext* ctx) {
    ...
    setLocation(deferStmt->location, ctx->start, ctx->stop);
    return deferStmt;
}
...
LgsVarDec* LgsParserAdapter::getVarDec(antlr4::tree::TerminalNode* name, const bool isConst, LgsExpr* expr) const {
    ...
    setLocation(varDec->location, name->getSymbol(), nullptr);
    return varDec;
}
...
LgsStmt* LgsParserAdapter::getIOStmt(LogosParser::IoStatementContext* ctx) {
...
LgsIfStmt* LgsParserAdapter::getIfStatement(LogosParser::IfStatementContext* ctx) {
    ...
    setLocation(ifStmt->location, ctx->start, ctx->stop);
    return ifStmt;
}
...
LgsForLoop* LgsParserAdapter::getForLoop(LogosParser::LoopStatementContext* ctx) {
    ...
    setLocation(loopStmt->location, ctx->start, ctx->stop);
...
LgsForLoop* LgsParserAdapter::getRangeLoop(LogosParser::RangeLoopContext* ctx) {
    ...
    setLocation(rangeLoop->location, ctx->start, ctx->stop);
    return rangeLoop;
}
...
LgsEnum* LgsParserAdapter::getEnum(LogosParser::EnumDeclarationContext* ctx) {
    ...
    setLocation(lgsEnum->location, ctx->start, ctx->stop);
...
LgsExpr* LgsParserAdapter::getJSON(LogosParser::JsonContext* ctx) {
    ...
    setLocation(json->location, ctx->start, ctx->stop);
    if (const auto constant = ctx->constant()) {
        const auto lgsConstant = getConstant(constant);
        if (constant->INTEGER() || constant->BOOL()) {
            json->intConst = lgsConstant->asIntConst();
        } else if (constant->FLOAT()) {
            json->floatConst = lgsConstant->asFloatConst();
        } else if (constant->STRING()) {
            json->strConst = lgsConstant->asStrConst();
        }
        json->type = lgsConstant->type;
    } else if (const auto null = ctx->NULL_()) {
        json->null = getNullValue(null);
        json->type = new LgsNullable(&LGS_ANY);
    } else if (const auto jsonObj = ctx->jsonObj()) {
        json->instance = getObjectFromJson(jsonObj);
        json->type = json->instance->type;
    } else if (const auto jsonArray = ctx->jsonArray()) {
        json->arr = getArrayExprFromJson(jsonArray);
        json->type = json->arr->type;
    }
    return json;
}

LgsExpr* LgsParserAdapter::getUnaryExpr(LogosParser::UnaryExprContext* ctx) {
...
LgsExpr* LgsParserAdapter::getPrefixExpr(LogosParser::PrefixExprContext* ctx) {
    ...
    setLocation(prefixExpr->location, ctx->start, ctx->stop);
    return prefixExpr;
}
...
LgsPostfixExpr* LgsParserAdapter::getPostfixExpr(LogosParser::PostfixExprContext* ctx) {
    ...
    setLocation(postfixExpr->location, ctx->start, ctx->stop);
    return postfixExpr;
}
...
LgsFuncCall* LgsParserAdapter::getFuncCall(LogosParser::FuncCallContext* ctx) {
    ...
    setLocation(funcCall->location, ctx->start, ctx->stop);
    return funcCall;
}

LgsExpr* LgsParserAdapter::getVectorExpr(LogosParser::VectorContext* ctx) {
    uint8_t dim = 0;
    if (ctx->VEC2()) dim = 2;
    ...
    setLocation(lgsVec->location, ctx->start, ctx->stop);
    for (const auto& expr : ctx->expr()) {
...
LgsExpr* LgsParserAdapter::getFirstSelection(LogosParser::SelectionContext* ctx) {
...
std::vector<LgsExpr*> LgsParserAdapter::getSelectionExprs(LogosParser::SelectionContext* ctx) {
...
LgsExpr* LgsParserAdapter::getConstant(LogosParser::ConstantContext* ctx) {
    ...
    if (constant) {
        setLocation(constant->location, ctx->start, ctx->stop);
    }
    return constant;
}
...
LgsInstance* LgsParserAdapter::getObjectFromJson(LogosParser::JsonObjContext* ctx) {
    const auto obj = new LgsObject("");
    const auto instance = new LgsInstance(obj);
    setLocation(obj->location, ctx->start, ctx->stop);
    setLocation(instance->location, ctx->start, ctx->stop);
    for (const auto jsonPair : ctx->jsonPair()) {
        const auto jsonExpr = getJSON(jsonPair->json());
        obj->addField(new LgsField("", jsonExpr->type, jsonExpr));
    }
    return instance;
}
...
LgsGroup* LgsParserAdapter::getGroup(LogosParser::GroupContext* ctx) {
    ...
    setLocation(group->location, ctx->start, ctx->stop);
...
LgsFuncType* LgsParserAdapter::getFuncType(LogosParser::FuncTypeContext* ctx) {
    ...
    setLocation(funcType->location, ctx->start, ctx->stop);
    funcType->rt = rt;
...
void LgsParserAdapter::setLocation(LgsLocation& location, const antlr4::Token* start, const antlr4::Token* stop) const {
    location.indexStart = start->getStartIndex();
    location.lineStart = start->getLine();
    location.posInLine = start->getCharPositionInLine() + 1;
    if (stop) {
        location.indexEnd = stop->getStopIndex() + 1;
    } else {
        location.indexEnd = start->getStartIndex() + start->getText().length() + 1;
    }
