...
void LgsC::compile() {
    ...
    if (jobs.empty()) {
        std::cout << "No jobs generated by driver" << std::endl;
        return;
    }
...
void LgsC::parse() const {
    const auto code = getFileText("../test.c");
...
void LgsDiagnosticsConsumer::HandleDiagnostic(const DiagnosticsEngine::Level diagLevel, const Diagnostic& info) {
    ...
    if (info.hasSourceManager()) {
        ...
        if (loc.isMacroID()) {
            ...
            os << sm.getFilename(loc) << ':' << sm.getSpellingLineNumber(loc) << ':' << sm.getSpellingColumnNumber(loc)
