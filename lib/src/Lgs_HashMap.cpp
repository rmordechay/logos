...
extern "C" void Lgs_Map_init(Lgs_HashMap* map, Lgs_TypeInfo* keyType, Lgs_TypeInfo* valueType) {
...
extern "C" Lgs_DArrayExpr* Lgs_Map_keys(const Lgs_HashMap* map) {
...
extern "C" Lgs_DArrayExpr* Lgs_Map_values(const Lgs_HashMap* map) {
    if (!map) return nullptr;
    const auto arr = new Lgs_DArrayExpr();
