...
extern "C" void Lgs_DArray_add(Lgs_DArrayExpr* arr, const void* value) {
    ...
    const auto size = arr->type->baseType->size;
...
extern "C" void Lgs_DArray_addInt(Lgs_DArrayExpr* arr, const int32_t value) {
    assert(arr);
    resizeArrIfNeeded(arr);
    *reinterpret_cast<int32_t*>(arr->data + arr->length * arr->type->baseType->size) = value;
    arr->length += 1;
}
...
extern "C" void Lgs_DArray_addLong(Lgs_DArrayExpr* arr, const int64_t value) {
    assert(arr);
    resizeArrIfNeeded(arr);
    *reinterpret_cast<int64_t*>(arr->data + arr->length * arr->type->baseType->size) = value;
    arr->length += 1;
}
...
extern "C" void Lgs_DArray_addSize(Lgs_DArrayExpr* arr, const size_t value) {
    assert(arr);
    resizeArrIfNeeded(arr);
    *reinterpret_cast<size_t*>(arr->data + arr->length * arr->type->baseType->size) = value;
    arr->length += 1;
}
...
extern "C" void Lgs_DArray_put(const Lgs_DArrayExpr* arr, const size_t index, const void* value) {
    ...
    std::memcpy(arr->data + index * arr->type->baseType->size, value, arr->type->baseType->size);
}
...
extern "C" void* Lgs_DArray_get(const Lgs_DArrayExpr* arr, const int64_t index) {
    ...
    return arr->data + actualIndex * arr->type->baseType->size;
}
...
extern "C" bool Lgs_DArray_contains(const Lgs_DArrayExpr* arr, const void* value) {
    ...
    const size_t len = arr->length / arr->type->baseType->size;
    const char* base = arr->data;
    for (size_t i = 0; i < len; ++i) {
        const void* elem = base + i * arr->type->baseType->size;
        bool match = false;
        if (arr->type->baseType->kind == RTT_DARRAY) {
        ...
        } else if (arr->type->baseType->kind == RTT_STR) {
            ...
            match = std::memcmp(elem, value, arr->type->baseType->size) == 0;
        }
...
static bool compare_arrays(const Lgs_DArrayExpr* arr1, const Lgs_DArrayExpr* arr2) {
    if (arr1->type->baseType->size != arr2->type->baseType->size) return false;
    if (arr1->type != arr2->type) return false;
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        if (arr1->type->baseType->kind == RTT_DARRAY) {
        ...
        } else if (arr1->type->baseType->kind == RTT_STR) {
            ...
            if (std::memcmp(elem1, elem2, arr1->type->baseType->size) != 0) return false;
        }
    }
    return true;
}
